...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      response = await fetch(`${AUTH_SERVICE_URL}/auth/register/client`, {
        method: "POST",
      ...
      console.error("Error registrando cliente:", getErrorMessage(fetchError));
      return NextResponse.json(
    ...
    if (data.token) {
      ...
      result.cookies.set("user_role", "cliente", { ...COOKIE_OPTIONS, maxAge: 604800 });
    }
    ...
    console.error("Error en registro de cliente:", getErrorMessage(error));

  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { message: data.message || "Error al registrar artista", errors: data.errors || [] },
          { status: response.status }
        );
      }

      const res = NextResponse.json(
        { success: true, user: data.user, message: "Cuenta de artista creada exitosamente" },
        { status: 201 }
      );
      ...
      res.cookies.set('user_role', 'artista', { ...cookieOpts, maxAge: 3600 });
    ...
    console.error("Error en registro de artista:", error.message);

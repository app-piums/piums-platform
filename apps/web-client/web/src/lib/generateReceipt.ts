...
export function generateBookingReceipt(data: ReceiptData, mode: 'print' | 'preview' = 'print') {
  ...
  const IOS_URL     = process.env.NEXT_PUBLIC_CLIENT_APP_IOS     || 'https://apps.apple.com/app/piums-cliente';
  const ANDROID_URL = process.env.NEXT_PUBLIC_CLIENT_APP_ANDROID || 'https://play.google.com/store/apps/details?id=io.piums.cliente';
  const LOGO_URL    = (typeof window !== 'undefined' ? window.location.origin : 'https://client.piums.io') + '/logo.png';
...
<div class="page">
  ...
  <div class="app-section">
    <p class="app-section-title">¿Aún no tienes la app? Descárgala gratis</p>

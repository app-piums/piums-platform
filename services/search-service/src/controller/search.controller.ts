...
export const searchController = {
  ...
  async smartSearch(req: Request, res: Response, next: NextFunction) {
    try {
      const { q, page, limit, city, country, minPrice, maxPrice } = smartSearchSchema.parse(req.query);
      const result = await searchService.smartSearch(q, { page, limit, city, country, minPrice, maxPrice });
      res.json(result);
    } catch (error) {
      next(error);
    }

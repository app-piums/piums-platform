...
| API-M4 | Medio | API/Auth | Ticket purchase sin límite de cantidad por order | Pendiente — requiere cambio de schema Prisma + lógica de validación |
| OPS-M1 | Medio | Ops | Crons en todos los pods — ejecución duplicada con HPA | Pendiente — requiere Redis distributed lock o pod dedicado |
...
| ID | Descripción | Fix concreto |
|---|---|---|
| API-M4 | Ticket purchase sin límite de cantidad por order | Validar `quantity <= maxPerOrder` (ej. 10) en schema de booking-service al crear compra de boleto |
| PII-M6 | JTI revocation: ningún middleware verifica si la sesión fue revocada | Lookup en DB por `jti` en cada request autenticado — todos los servicios con auth middleware |
| OPS-M1 | Crons corren en todos los pods con HPA — ejecución duplicada | Redis distributed lock (`SET cron:lock NX EX 60`) en `booking-service/cron.service.ts` y `users-service/purge.service.ts` |
